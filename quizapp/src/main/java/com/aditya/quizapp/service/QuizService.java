...
public class QuizService {
    ...
    public ResponseEntity<List<QuestionWrapper>> getQuestions(Integer id) {
        ...
        for(Question q : questionsFromDB){
            QuestionWrapper qw=new QuestionWrapper(q.getId(), q.getCategory(),q.getDifficulty_level(),q.getOption1(), q.getOption2(), q.getOption3(), q.getOption4(), q.getQuestion_title());
    ...
    public ResponseEntity<Integer> submitQuiz(Integer id, List<Response> responses) {
        ...
        for(Response response : responses){
            if(response.getResponse().equals(questions.get(i).getCorrect_answer())) right+=1;
            i+=1;
        }
